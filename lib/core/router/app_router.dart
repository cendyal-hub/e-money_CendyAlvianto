...
class AppRouter {
  ...
  static GoRouter get router => GoRouter(
        ...
        routes: [
          ...
          // Deep link endpoint from external apps (e.g. Marketplace)
          GoRoute(
            path: '/pay',
            builder: (context, state) {
              final amountStr = state.uri.queryParameters['amount'] ?? '0';
              final amount = double.tryParse(amountStr) ?? 0.0;
              final orderId = state.uri.queryParameters['orderId'] ?? 'External Payment';
              final callback = state.uri.queryParameters['callback'];
              
              return _withPayment(PinPage(flowData: {
                'kind': 'deeplink',
