    ...
    return BlocListener<PaymentBloc, PaymentState>(
      listener: (context, state) {
        if (state is PaymentTransferSuccess) {
          ...
          if (flow['kind'] == 'deeplink' && callback != null) {
            // Redirect back to caller app, then navigate E-Money to home
            // (JANGAN gunakan SystemNavigator.pop() karena itu mematikan proses
            // secara paksa dan menyebabkan Marketplace restart dari awal → logout)
            final uri = Uri.parse('$callback?status=success&ref=DKG${result.transactionId}');
            launchUrl(uri, mode: LaunchMode.externalApplication);
            // Refresh saldo sebelum kembali ke home
            if (context.mounted) {
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        // Tampilkan detail pembayaran jika dari deeplink
                        if (widget.flowData['kind'] == 'deeplink') ...[
                          ...
                          Container(
                            ...
                            child: Column(
                              children: [
                                Text(
                                  widget.flowData['description'] as String? ?? 'Pembayaran',
                                ...
                                Text(
                                  CurrencyFormatter.format((widget.flowData['amount'] as num? ?? 0).toDouble()),
                                  style: const TextStyle(
                                    fontFamily: 'PlusJakartaSans',
                                    fontSize: 22,
                                    fontWeight: FontWeight.w800,
