...
Future<void> init() async {
  ...
  // Local datasource (register dulu agar bisa dipakai sebelum digunakan oleh datasource lain)
  sl.registerLazySingleton<SecureStorageDatasource>(
    () => SecureStorageDatasourceImpl(secureStorage),
  );

  // --- Muat token JWT yang tersimpan sejak sesi sebelumnya ---
  // Ini krusial agar deep link dari aplikasi luar (marketplace) bisa langsung
  // melakukan transaksi tanpa user harus login ulang.
  final storedToken = await secureStorage.read(key: 'jwt_token');
  if (storedToken != null) {
    sl<ApiClient>().setAuthToken(storedToken);
  }
